...
mod tests {
    ...
    async fn emails_json() {
        ...
        let es = Arc::new(RwLock::new(EmailStore::new()));
        {
            ...
            _ = esw
                .push(ReceivedEmail::new(create_send_email_input(Some(
        ...
        let resp: Vec<ReceivedEmail> = serde_json::from_slice(&body_bytes).unwrap();
        assert_eq!(&resp, es.read().await.get_all());
    }
    ...
    async fn emails_html() {
        ...
        assert_eq!(
            resp,
            crate::routes::local::emails::html::templates::emails::build(esr.get_all(), None)
                .into_string()
        );
    }
    ...
    async fn emails_sse() {
        ...
        let mut esr = app_state.email_store.write().await;
        let rec_email1 =
            ReceivedEmail::new(create_send_email_input(Some("a@example.com".to_string())));
        let rec_email2 =
            ReceivedEmail::new(create_send_email_input(Some("b@example.com".to_string())));
        _ = esr.push(rec_email1.clone()).await;
        _ = esr.push(rec_email2.clone()).await;
        ...
        let expected = vec![
            Event {
                ...
                data: crate::routes::local::emails::html::templates::email_row::build(&rec_email1)
                    .into_string(),
                id: "".to_string(),
                retry: None,
            },
            Event {
                event: "email".to_string(),
                data: crate::routes::local::emails::html::templates::email_row::build(&rec_email2)
                    .into_string(),
                id: "".to_string(),
                retry: None,
            },
    ...
    async fn email_json() {
        ...
        let body_bytes = to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let resp: ReceivedEmail = serde_json::from_slice(&body_bytes).unwrap();
        assert_eq!(
            &resp,
            es.read().await.get_by_message_id(&message_id).unwrap()
        );
    }
        ...
        assert_eq!(
            ...
            crate::routes::local::emails::html::templates::emails::build(
                ...
                Some(crate::routes::local::emails::html::templates::email::build(
                    esr.get_by_message_id(&message_id).unwrap()
                )),
            )
            .into_string()
        );
